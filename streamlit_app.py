...
st.markdown("""
### Important Note
- For **101-feature inputs**: The x_Bj target value entered below must be between 1e-7 and 1e-2
- For **103-feature inputs**: The last value in your array (x_Bj target) must satisfy 1e-7 < x ≤ 1e-2
""")

# File uploader
uploaded_file = st.file_uploader("Upload .pkl file (101 or 103 features)", type=["pkl"])

# x_Bj target input (only used for 101-feature files)
x_bj_target = st.number_input(
    "x_Bj Target (only used if uploading 101 features)", 
    value=1e-3, 
    min_value=1e-7, 
    max_value=1e-2,
    format="%e"
...
if uploaded_file:
    try:
        ...
        if X_input.shape[1] == 101:
            ...
            st.success("Using provided x_Bj target (validated range)")
            ...
            if np.any((x_bj_uploaded <= 1e-7) | (x_bj_uploaded > 1e-2)):
                st.error("⚠️ Last value in 103-feature array must be 1e-7 < x_Bj ≤ 1e-2")
                st.stop()
